...
const btnNavMenu= document.querySelector('#nav-menu');                        //Pulsante menu della navbar
...
//Listener per il bottone della navbar
...
function onNextCont(){
    //Controllo se ho più di 4 contenuti che possono essere visualizzati (dovuti a caricamento/ricerca)
    if(viewableContent.length > 4) 
    {    
        //Della lista mostro solo 4 contenuti per volta quindi logicamente devo lavorare con una "finestra" 
        //di dimensione 4                              
        if(contentsOffset < viewableContent.length-4) 
...
function remFav(event)
{
    ...
    //Se non ho preferiti nascondo la sezione preferiti
...
//Aggiungo il listener al pulsante della navbar
btnNavMenu.addEventListener('click', openNavMenu);
